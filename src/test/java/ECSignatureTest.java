...
public class ECSignatureTest {
    ...
    @Test
    public void ECSignatureTest() throws Exception{
        for(int keySize : sizes){
            for(Digest digest : digests){
                for(PaddingMode paddingMode : paddingModes){
                    KeyAuthorizations keyAuthorizations = TestUtils.getKeyAuthorizations(keySize, paddingMode, BlockMode.ECB, Purpose.SIGN, Algorithm.EC);
                    keyAuthorizations.setCurveName(Utils.getEcCurveFromKeySize(keySize).value());
